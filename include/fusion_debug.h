...
static cv::Mat dbg_normals(const std::vector<float3> &n, int W, int H)
{
    ...
    cv::resize(img, img, cv::Size(W / 2, H / 2), 0, 0, cv::INTER_NEAREST);
    return img;
}

static cv::Mat dbg_color_input(const cv::Mat &color_or_gray,
                               const cv::Mat &depth_m)
{
    cv::Mat img;
    if (!color_or_gray.empty())
    {
        if (color_or_gray.channels() == 1)
            cv::cvtColor(color_or_gray, img, cv::COLOR_GRAY2BGR);
        else if (color_or_gray.channels() == 3)
            img = color_or_gray.clone();
        else if (color_or_gray.channels() == 4)
            cv::cvtColor(color_or_gray, img, cv::COLOR_BGRA2BGR);
    }
    else if (!depth_m.empty())
    {
        ...
        if (max_v > min_v)
        {
            depth_m.convertTo(gray, CV_8UC1, -255.0 / (max_v - min_v),
    ...
    if (img.empty())
        img = cv::Mat(120, 320, CV_8UC3, cv::Scalar(20, 20, 20));
    cv::putText(img,
                color_or_gray.empty() ? "input intensity: depth fallback"
                                      : "input intensity / RGB",
                         ...
                         const std::vector<DualQuat> *transforms = nullptr)
{
    ...
    auto project = [&](float3 p, cv::Point &q)
    {
        ...
        if (p.z <= 0.01f)
            return false;
        q = {(int)(cam.fx * p.x / p.z + cam.cx),
             (int)(cam.fy * p.y / p.z + cam.cy)};
    ...
    if (nodes && transforms)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            float3 pi = dq_transform_point(
                dq_centered((*transforms)[i], (*nodes)[i].pos), (*nodes)[i].pos);
            cv::Point a;
            if (!project(pi, a))
                continue;
            ...
            cv::circle(color, a, 2, cv::Scalar(0, 250, 255), -1, cv::LINE_AA);
        }
    }
    else
    {
        std::cout << "Error retrieving nodes/transforms for debug visualization" << std::endl;
        exit(1);
    }
    cv::putText(color, "live surface", cv::Point(8, 18), cv::FONT_HERSHEY_SIMPLEX,
                0.45, cv::Scalar(220, 220, 220), 1);
    return color;
}
                         ...
                         int n_valid)
{
    ...
    for (const auto &c : corrs)
    {
        ...
        if (src.z > 0.01f && dst.z > 0.01f && drawn % 80 == 0)
        {
            ...
            if (u1 >= 0 && u1 < cam.width && v1 >= 0 && v1 < cam.height && u2 >= 0 &&
                ...
                cv::line(img, cv::Point(u1, v1), cv::Point(u2, v2),
    ...
    cv::putText(grown,
                std::string("G=src R=dst valid=") + std::to_string(n_valid) +
                    std::string(" shown=") + std::to_string(drawn),
                cv::Point(8, 18), cv::FONT_HERSHEY_SIMPLEX, 0.45,
                cv::Scalar(220, 220, 220), 1);
    return grown;
}

static cv::Mat dbg_sift_matches(const cv::Mat &gray,
                                const std::vector<cv::Point2f> &src_pts,
                                const std::vector<cv::Point2f> &dst_pts,
                                const std::vector<unsigned char> &accepted)
{
    cv::Mat img;
    if (gray.empty())
        img = cv::Mat(480, 640, CV_8UC3, cv::Scalar(20, 20, 20));
    else if (gray.channels() == 1)
        cv::cvtColor(gray, img, cv::COLOR_GRAY2BGR);
    else
        img = gray.clone();

    for (size_t i = 0; i < src_pts.size() && i < dst_pts.size(); ++i)
    {
        bool ok = i < accepted.size() && accepted[i];
        if (ok)
        {
            cv::circle(img, src_pts[i], 2, cv::Scalar(255, 160, 40), -1, cv::LINE_AA);
            cv::circle(img, dst_pts[i], 3, cv::Scalar(80, 255, 80), -1, cv::LINE_AA);
            cv::line(img, src_pts[i], dst_pts[i], cv::Scalar(80, 255, 80), 1,
                     cv::LINE_AA);
        }
        else
        {
            cv::drawMarker(img, dst_pts[i], cv::Scalar(80, 80, 255),
                           cv::MARKER_TILTED_CROSS, 7, 1, cv::LINE_AA);
        }
    }

    cv::putText(img, "SIFT: green=accepted constraints, red x=rejected",
                cv::Point(8, 18), cv::FONT_HERSHEY_SIMPLEX, 0.45,
                cv::Scalar(220, 220, 220), 1);
    return img;
}
