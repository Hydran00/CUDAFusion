...
__device__ __forceinline__
float norm3(float3 v) { return sqrtf(dot3(v, v)); }
...
__device__ __forceinline__
Mat4 exp_se3(const float* twist) {
...
__device__ __forceinline__
float node_weight(float3 p, float3 node_pos, float radius) {
...
{
    // ICP punto-piano: residuo = n^T (warped_p - dst)
    // ∂r / ∂(twist_k) = w_k * [n^T [warped_p]× | n^T]
...
__device__ __forceinline__
bool invert6x6_cholesky(const float* A, float* Ainv) {
